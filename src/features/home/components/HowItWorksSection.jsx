    ...
    <section className="py-16 md:py-24 bg-white relative overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="mt-16 flex flex-col md:flex-row justify-between items-start md:items-center relative">
          ...
          {steps.map((step, idx) => (
            <div key={idx} className="relative z-10 flex flex-col items-center mb-12 md:mb-0 w-full md:w-1/4">
              <div className="w-24 h-24 bg-white border-4 border-primary text-primary transition-colors hover:bg-primary hover:text-white rounded-full flex items-center justify-center text-3xl font-bold shadow-lg mb-6 cursor-default">
                {step.id}
              </div>
              <h3 className="text-xl font-bold text-brown-900 mb-2">{step.title}</h3>
              <p className="text-gray-500 text-center px-4 md:px-2">{step.desc}</p>
            </div>
          ))}
        </div>
      </div>
